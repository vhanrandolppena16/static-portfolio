  ...
  return (
    ...
    >
      <div className='text-center mb-8'>
        <h2 className='text-7xl sm:text-4xl font-bold text-white'>
          PROJECTS
        </h2>

        <div className='w-36 h-1 bg-green-900 mx-auto mt-2'/>
      ...
      <div className="grid gap-12 grid-cols-1 [@media(min-width:900px)_and_(max_width:1159px)]:grid-cols-2 [@media(min-width:1200px)]:grid-cols-3 w-full h-full">
        {projects.map((project) =>
          <div key={project.id}
            ...
            className="border border-white bg-gray-900 backdrop-blur-md 
            rounded-2xl overflow-hidden cursor-pointer
            hover:shadow-purple-500/50 hover:-translate-y-4 transition-transform duration-300"
          >
            <div className="p-4">
              <img
                src={project.image}
            ...
            <div className="p-6">
              ...
              <p className="text-gray-500 mb-4 pt-4 lime-clamp-3">
                {project.description}
              </p>
              <div className="mb-4">
                {project.tags.map((tag, index) => (
                  <span
                    key={index}
                    className="inline-block bg-green-400/10 text-xs font-semibold text-gray-500 rounded-full px-2 py-1 mr-2 mb-2 "
                  >
                    {tag}
                  </span>
                ))}
              ...
              <div className="lg:p-8 p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {selectedProject.tags.map((tag, index) => (
                    <span
                      key={index}
                      className="bg-[#99f29d] text-xs font-semibold text-black rounded-full px-2 py-1"
                    >
                      {tag}
                    </span>
                  ))}
